-- Blog Automation 전체 스키마 설계 (PostgreSQL 최적화)
...
-- 인덱스 생성
CREATE INDEX IF NOT EXISTS idx_schedule_date ON blog_automation.monthly_publishing_schedule (year, month, day);
CREATE INDEX IF NOT EXISTS idx_schedule_site ON blog_automation.monthly_publishing_schedule (site);
CREATE INDEX IF NOT EXISTS idx_schedule_status ON blog_automation.monthly_publishing_schedule (status);

CREATE INDEX IF NOT EXISTS idx_content_site_status ON blog_automation.content_files (site, status);
CREATE INDEX IF NOT EXISTS idx_content_created ON blog_automation.content_files (created_at);
CREATE INDEX IF NOT EXISTS idx_content_published ON blog_automation.content_files (published_at);

CREATE INDEX IF NOT EXISTS idx_trending_week_site ON blog_automation.trending_topics (week_start_date, site);
CREATE INDEX IF NOT EXISTS idx_trending_category ON blog_automation.trending_topics (category);
CREATE INDEX IF NOT EXISTS idx_trending_priority ON blog_automation.trending_topics (priority DESC);

CREATE INDEX IF NOT EXISTS idx_cache_api_key ON blog_automation.api_cache (api_name, cache_key);
CREATE INDEX IF NOT EXISTS idx_cache_expires ON blog_automation.api_cache (expires_at);

CREATE INDEX IF NOT EXISTS idx_history_site_date ON blog_automation.publish_history (site, published_at);
CREATE INDEX IF NOT EXISTS idx_history_status ON blog_automation.publish_history (publish_status);

CREATE INDEX IF NOT EXISTS idx_status_component ON blog_automation.system_status (component);
CREATE INDEX IF NOT EXISTS idx_status_checked ON blog_automation.system_status (checked_at DESC);

CREATE INDEX IF NOT EXISTS idx_logs_job_status ON blog_automation.scheduler_logs (job_name, status);
CREATE INDEX IF NOT EXISTS idx_logs_started ON blog_automation.scheduler_logs (started_at DESC);

CREATE INDEX IF NOT EXISTS idx_revenue_site_date ON blog_automation.revenue_data (site, date);
CREATE INDEX IF NOT EXISTS idx_revenue_date ON blog_automation.revenue_data (date DESC);

CREATE INDEX IF NOT EXISTS idx_images_content ON blog_automation.image_files (content_file_id);
CREATE INDEX IF NOT EXISTS idx_images_type ON blog_automation.image_files (image_type);

CREATE INDEX IF NOT EXISTS idx_keyword_name ON blog_automation.keyword_analysis (keyword);
CREATE INDEX IF NOT EXISTS idx_keyword_category ON blog_automation.keyword_analysis (category);
CREATE INDEX IF NOT EXISTS idx_keyword_trend ON blog_automation.keyword_analysis (trend_score DESC);
...
-- 트리거 생성
CREATE OR REPLACE TRIGGER update_monthly_schedule_updated_at
...
CREATE OR REPLACE TRIGGER update_keyword_analysis_updated_at
